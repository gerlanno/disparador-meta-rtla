...
from controller.controller import get_titulos, get_business_account, get_templates, historico_disparos, cadastrar_template
...
def disparar(business_acc_name):
    ...
    try:
        ...
        print(template_name, "primeira definição do template")
    ...
    if titulos and template_name:
        for titulo in titulos:
            nome_devedor,titulo_id,nome_credor,valor_titulo,numero_titulo,url_cartorio,nome_cartorio,telefones = titulo            
            ...
            if telefones != None:          
                for telefone in telefones:
                    print(template_name, "Dentro do loop telefone")
                    # Enviar a mensagem para o número de cadastro do titulo.
                    send_messages(phone_id, api_token, telefone, template_name, titulo_id, paramentros_template, business_id)
    else:
...
def send_messages(phone_id, api_token, telefone, template_name, titulo_id, paramentros_template, business_id):
    ...
    if status_code == 200:
        ...
        historico_disparos(messageid=messageid, titulo_id=int(titulo_id), whatsapp=telefone, wa_id=wa_id, message_status=message_status, response=str(response_text))
        ...
        if error_code in ["132015", "132016"]:            
            ...
            cadastrar_template(name=template_name, wbaccount_id=business_id, status="DISABLED")
            template_name = set_template(business_id)
        historico_disparos(titulo_id=int(titulo_id), whatsapp=telefone,response=str(response_text))
